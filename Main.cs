...
[BepInPlugin("cheese.csa.thunder", "Custom Scenario Assets", "2.0.0")]
public class CustomScenarioAssets : BaseUnityPlugin
{
    ...
    public void Awake()
    {
        Harmony harmony = new Harmony("cheese.customScenarioAssets");
    ...
    {
        Debug.Log("Searching for .csa files in the legacy mods folder");
        ...
        Debug.Log("Searching for .csa files in the new mods folder");
        string newAddress = Path.GetDirectoryName(instance.Info.Location) + @"\..\";
        Debug.Log("Checking for: " + newAddress);

        if (Directory.Exists(newAddress))
        {
            Debug.Log(newAddress + " exists!");
            DirectoryInfo info = new DirectoryInfo(newAddress);

            Debug.Log("Searching " + newAddress + info.Name + " for .csa");
            foreach (FileInfo file in info.GetFiles("*.csa"))
            {
                Debug.Log("Found " + file.FullName);
                StartCoroutine(LoadAssetBundle(file));
            ...
            foreach (DirectoryInfo directory in info.GetDirectories())
            {
                Debug.Log("Searching " + newAddress + directory.Name + " for .csa");
                foreach (FileInfo file in directory.GetFiles("*.csa"))
                {
                    Debug.Log("Found " + file.FullName);
                    StartCoroutine(LoadAssetBundle(file));
        ...
        {
            Debug.Log(newAddress + " doesn't exist.");
        }
